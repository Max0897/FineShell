...
| AI 助手  | OpenAI 兼容接口、服务预设与模型发现、流式对话、多文件工作区上下文、可关闭的只读诊断工具（状态、进程、目录、网络连接、需确认的 Ping 与路由追踪）、可逐项审阅、批量应用和安全回滚的远程文件内容修改、新建、重命名与删除提案、带本地风险分级、手动提交关联和结果分析的结构化终端命令提案、精简变更历史及对话历史 |
